...
for i in mat:
    ...
    process = Popen(["../bin/heston-pricing", "--iters", str(50000), "--maturity", str(i)], stdout=PIPE)
    ...
    startbns = time.time()
    process = Popen(["../bin/heston-pricing", "--model", "bns",  "--meanrevert", "0", "--correlation", "0", "--elasticity", "1",  "--iters", str(50000), "--maturity", str(i)], stdout=PIPE)
    (_, _) = process.communicate(); _ = process.wait()
    stopbns = time.time()
    x.append(i)
