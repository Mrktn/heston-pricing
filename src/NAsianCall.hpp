...
public:
    NAsianCall(double s0, double r, double T, double K, double rho, double k, double theta, double dzeta) : s0(s0), r(r), T(T), K(K), rho(rho), k(k), theta(theta), dzeta(dzeta), G(0, 1) {
    ...
    double simulate(unsigned n_iter, std::function<double(unsigned)> const & gamma, std::function<double(unsigned)> const & eta, std::mt19937_64 & gen) {
        ...
        for (unsigned k_iter = 0; k_iter < n_iter; ++k_iter) {
            ...
            double trueI = (I - prefix[k_iter]) / Xi[k_iter];
            ...
            double F = std::exp(-r * T) * std::max(amended_trueI / T - K, 0.0);
